...
class FaceAging(object):
    def __init__(self,
                 ...
                 test_savedir='./saveTest'
                 ):
        self.test_savedir = test_savedir;
        ...
        for epoch in range(num_epochs):
            ...
            for ind_batch in range(num_batches):
                ...
                for i, label in enumerate(batch_files):
                    label = int(str(batch_files[i]).split(os.path.sep)[-1].split('_')[0])
                    if 0 <= label <= 5:
                        label = 0
                    elif 6 <= label <= 10:
                        label = 1
    ...
    def discriminator_img(self, image, y, gender, is_training=True, reuse_variables=False,
    ...
    def custom_test(self, testing_samples_dir=None, file_names=None, gender=1):
        """

        :param testing_samples_dir:
        :param file_names:
        :param gender:   1 表示male  2表示female 3表示male female
        :return:
        """
        print("testing_samples_dir=%s file_names=%s" % (testing_samples_dir, file_names))
        ...
        show_col = len(file_names)
        if show_col < num_samples:
            print('The number of testing images is must larger than %d' % num_samples)
            # exit(0)
            # file_names = np.repeat(file_names, 10)
            while (len(file_names) < 10):
        ...
        if gender == 1:
            ...
            self.test(images, gender_male, 'test_as_male.png', show_col)
        elif gender == 2:
            gender_female = np.ones(
                shape=(num_samples, 2),
                dtype=np.float32
            ) * self.image_value_range[0]
            for i in range(gender_female.shape[0]):
                gender_female[i, 1] = self.image_value_range[-1]
            self.test(images, gender_female, 'test_as_female.png', show_col)
        else:
            gender_male = np.ones(
                shape=(num_samples, 2),
                dtype=np.float32
            ...
            for i in range(gender.shape[0]):
                gender[i, 0] = self.image_value_range[-1]  # //男
                gender[i, 1] = self.image_value_range[-1]  # //女
            # self.test(images, gender, 'test_result.png', show_col)
